...
v1 - Malignant_MOUSE
myeloid_cells_v1 - myeloid_cells_v1
...
seu <- SetupForWGCNA(
  ...
  # group_by = 'annotation',
  wgcna_name = "myeloid_cells_v1" # the name of the hdWGCNA experiment
)
```
...
seu <- ConstructNetwork(
  seu,
  tom_name = 'Myeloid_MOUSE', # name of the topoligical overlap matrix written to disk
  overwrite_tom = TRUE
)
```
...
seu <- ModuleConnectivity(
  seu,
  group.by = 'annotation', group_name = "Myeloid_cells"
)
```
...
ModuleRadarPlot(
  seu,
  group.by = 'age',
  barcodes = seu@meta.data %>% subset(annotation == 'Myeloid_cells') %>% rownames(),
  axis.label.size=4,
  grid.label.size=4
)
```
...
```{r}
group1 <- seu@meta.data %>% subset(annotation == 'Myeloid_cells' & age == 'young') %>% rownames
group2 <- seu@meta.data %>% subset(annotation == 'Myeloid_cells' & age == 'adult') %>% rownames
...
```{r eval=FALSE, include=FALSE}
qsave(seu, paste0(PREPRDATADIR, 'seu_WGCNA_myeloid.qs'))
```

```{r eval=FALSE, include=FALSE}
seu <- qread(paste0(PREPRDATADIR, 'seu_WGCNA_myeloid.qs'))
```
...
atlas_myel <- ProjectModules(
  subset(atlas, Annotation == 'Myeloid_cells'),
  seurat_ref = seu,
  ...
  wgcna_name_proj = "myeloid_cells_v1",
  modules = modules
)

# compute module hub scores for projected modules:
atlas_myel <- ModuleExprScore(
  atlas_myel,
  n_genes = 50,
  method='Seurat'
)
...
```{r eval=FALSE, include=FALSE}
qsave(atlas_myel, paste0(PREPRDATADIR, 'atlas_myel_WGCNA_mouse.qs'))
```

```{r eval=FALSE, include=FALSE}
atlas_myel <- qread(paste0(PREPRDATADIR, 'atlas_myel_WGCNA_mouse.qs'))
```
...
atlas_myel$less18M <- sample_metadata[clean_sample_id(atlas_myel$SampleID),]$less18M
```

## DE

```{r}
group1 <- atlas_myel@meta.data %>% subset(Annotation == 'Myeloid_cells' & less18M == TRUE) %>% rownames
group2 <- atlas_myel@meta.data %>% subset(Annotation == 'Myeloid_cells' & less18M == FALSE) %>% rownames
```
...
```{r}
# Merge the two data frames by module
merged_df <- merge(DMEs_human, DMEs, by = "module", suffixes = c("_human", "_mouse"))

# Identify modules with the same direction
same_direction <- merged_df[sign(merged_df$avg_log2FC_human) == sign(merged_df$avg_log2FC_mouse), ]
