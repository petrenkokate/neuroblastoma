...
```{r fig.height=19}
DimHeatmap(t_cells, dims = 1:20, cells = 500, balanced = TRUE)
```
...
p4 <- CellScatter(t_cells_bulk, "CD4-CTLA4-Tcells_young", "CD4-CTLA4-Tcells_adult")
p4 <- CellScatter(t_cells_bulk, "CD4-CTLA4-Tcells_young", "CD4-CTLA4-Tcells_adult", highlight = p4$data %>% filter(abs(`CD4.CTLA4.Tcells_young` - `CD4.CTLA4.Tcells_adult`) > 1) %>% rownames())
p4 <- LabelPoints(plot = p4, points =  p4$data %>% filter(abs(`CD4.CTLA4.Tcells_young` - `CD4.CTLA4.Tcells_adult`) > 1) %>% rownames(), repel = TRUE)
...
cd4_cells.de.markers <- FindMarkers(t_cells, 
                                  ...
                                  min.pct = 0.25)
ctla_cells.de.markers <- FindMarkers(t_cells, 
                                  ident.1 = "CD4_CTLA4_Tcells_young", ident.2 = "CD4_CTLA4_Tcells_adult", 
                                  min.pct = 0.25)
...
plot_grid(volcano_plot(nk_cells.de.markers) +
          ...
          volcano_plot(cd4_cells.de.markers) +
            ggtitle('CD4+ T cells') +
          volcano_plot(ctla_cells.de.markers) +
            ggtitle('CD4+CTLA4+ T cells'),
          nrow = 1, rel_widths = c(0.5, 0.5, 1, 1))
```
...
malig_cells <- escape::runEscape(malig_cells, 
                        ...
                        groups = 6000, 
                        min.size = 5,
                        new.assay.name = "tf.GSVA")

qsave(malig_cells, paste0(PREPRDATADIR, 'malig_cells_mouse_gsvas3.qs'))
```
