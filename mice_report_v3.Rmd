...
plot_annotation <- plot_grid(DimPlot(seu, group.by='annotation', cols = c("#FF69B4FF", "#40E0D0FF", "#6495EDFF",  "#9ACD32FF", "#BA55D3FF", "#DDA0DDFF", "#FFA07AFF")) +
  ggtitle("Annotation"), 
  DimPlot(seu, group.by = 'age', cols = c("#6495EDFF", "#FF69B4FF")) +
  ggtitle("Age"),
  nrow = 1, rel_widths = c(1, 0.9), labels = c('A', 'B'))

Idents(object = seu) <- "annotation"
clusters.markers <- FindAllMarkers(seu, only.pos = TRUE, min.pct = 0.25)
...
plot_markers <- DotPlot(seu, top5$gene, cols = c('lightgrey', "#BA55D3FF")) +
  RotatedAxis()

plot_compostion <- plot_grid(dittoBarPlot(seu, 'age', group.by = 'annotation', color.panel = my_colors) +
                               ggtitle('Age'),
          dittoBarPlot(seu, 'Phase', group.by = 'annotation', color.panel = my_colors),
          dittoBarPlot(seu, 'annotation', group.by = 'age', color.panel = my_colors) +
            ggtitle('Annotation'),
...
plot_de <- plot_grid(volcano_plot(malignant.de.markers) +
            ggtitle('Malignant cells') +
              NoLegend(),
            volcano_plot(t_cells.de.markers) +
            ggtitle('T-cells') +
          ...
          volcano_plot(myeloid.de.markers) +
            ggtitle('Myeloid cells') +
            NoLegend(),
          nrow = 1, labels = c('G', 'H', 'I', 'K'))

plot_grid(plot_annotation, 
          plot_markers,
          plot_compostion,
          plot_de,
          nrow = 4, labels = c('', 'C', '', ''),
          rel_heights = c(1.8, 1.5, 1.5, 2.4))

DimPlot(seu, group.by='RNA_snn_res.0.1')
DimPlot(seu, group.by='annotation', cols = c("#FF69B4FF", "#40E0D0FF", "#6495EDFF",  "#9ACD32FF", "#BA55D3FF", "#DDA0DDFF", "#FFA07AFF")) +
  ggtitle("Annotation")
```

# Immunesuppression

Neuroblastoma secretes a variety of immunosuppressive molecules, including Gal-1, MIF, sGD2, sHLA-E, TGFβ, sMICA, sB7-H6 and arginase-2, which can impede TIL function. 
the tumours suppressed immunity by enhancing the expression of PD-L1 and infiltration of Tregs, supporting findings in human neuroblastoma
TGFβ, galectin-1 and MIF are well-known immunosuppressive molecules produced by neuroblastoma, which can impair CTL and/or NK cell function
...
VlnPlot(malig_cells, c('Cd274', 'Psmb9', 'Psmb8', 'Tap1', 'Tap2', 'B2m', 'H2-Q7', 'Tgfb1', 'Il10', 'Cd163', 'Arg1', 'Nos2', 'Cybb', 'Nox2', 'Ctla4', 'Epas1'), cols=c("#FF69B4FF", "#6495EDFF"))
```
...
maligcells.markers %>% 
  dplyr::filter(cluster == 'adult')

ego_malig <- enrichGO(gene = maligcells.markers %>% 
                         dplyr::filter(cluster == 'adult') %>% 
...
barplot(ego_malig, showCategory=20, font.size = 14) +
  scale_fill_gradient(low = "#FF69B4FF", high = "#6495EDFF") +
  theme(plot.title = element_text(size=22),
        legend.text = element_text(size=12),
         legend.title = element_text(size=14),
...
goplot(ego_malig, showCategory = 200)

hallmark.markers
```

```{r}
FeaturePlot(seu, c('Th','Cyp11b1', 'Scarb1', 'Phox2b'))
```
