...
WORKDIR = '~/Desktop/lab/neuroblastoma/'
PREPRDATADIR = '~/Desktop/lab/neuroblastoma/preprocessing_data/'
...
library(enrichplot)
create_emap <- function(enrichment_result, title) {
  if(is.null(enrichment_result) || nrow(enrichment_result) == 0) {
    return(NULL)
  }
  
  edo <- pairwise_termsim(enrichment_result)
...
for(celltype in names(pathway_results)) {
  if(!is.null(pathway_results[[celltype]])) {
    tryCatch({
      # GO enrichment map
      go_emap <- create_emap(pathway_results[[celltype]]$go, 
                           paste("GO Enrichment Map -", celltype))
      if(!is.null(go_emap)) print(go_emap)
      
      # KEGG enrichment map
      kegg_emap <- create_emap(pathway_results[[celltype]]$kegg,
                             paste("KEGG Enrichment Map -", celltype))
      if(!is.null(kegg_emap)) print(kegg_emap)
      
      # Reactome enrichment map  
      reactome_emap <- create_emap(pathway_results[[celltype]]$reactome,
                                 paste("Reactome Enrichment Map -", celltype))
      if(!is.null(reactome_emap)) print(reactome_emap)
    }, error = function(e) {
      message(paste("Error in", celltype, ":", e$message))
    })
  }
}
```
...
mouse <- qread('~/Desktop/lab/neuroblastoma/data/seu_mouse.qs')
...
```{r}
create_expression_boxplots <- function(bulk, genes, group_by = c("dataset", "less18M"), 
                                       facet_by = "Annotation", 
                                       ncol = 3) {
  # Get normalized data
  expression_data <- GetAssayData(bulk, slot = "data")
  
  # Check if genes exist in the dataset
  genes <- intersect(genes, rownames(expression_data))
  if(length(genes) == 0) {
    stop("None of the specified genes found in the dataset")
  }
  
  # Create long format data frame for plotting
  plot_data <- data.frame(t(expression_data[genes, , drop = FALSE]))
  plot_data$dataset <- bulk$dataset
  plot_data$less18M <- bulk$less18M
  plot_data$Annotation <- bulk$Annotation
  
  # Melt data for plotting
  plot_data_long <- reshape2::melt(plot_data, 
                                  id.vars = c("dataset", "less18M", "Annotation"),
                                  variable.name = "gene",
                                  value.name = "expression")
  
  # Create basic boxplot
  p <- ggplot(plot_data_long, aes(x = dataset, y = expression, fill = less18M)) +
    geom_boxplot(outlier.size = 0.5, alpha = 0.7) +
    facet_wrap(~gene + get(facet_by), scales = "free_y", ncol = ncol) +
    theme_minimal() +
    theme(
...
# Function to test for differential expression across conditions
test_gene_expression <- function(bulk, genes, cell_types = NULL) {
  # Get normalized data
  expression_data <- GetAssayData(bulk, slot = "data")
  ...
  if(is.null(cell_types)) {
    cell_types <- unique(bulk$Annotation)
  }
  ...
  for(ct in cell_types) {
    ...
    # For each dataset
    for(ds in unique(bulk$dataset)) {
      # Subset data
      subset_idx <- bulk$Annotation == ct & bulk$dataset == ds
      if(sum(subset_idx) > 0) {
        
        # Get expression data
        expr <- expression_data[genes, subset_idx, drop = FALSE]
        groups <- bulk$less18M[subset_idx]
        
        # Only test if we have both groups
        if(length(unique(groups)) > 1) {
          # Run Wilcoxon test for each gene
          test_results <- lapply(genes, function(g) {
            test <- wilcox.test(expr[g,] ~ groups)
            data.frame(
              gene = g,
              celltype = ct,
              dataset = ds,
          ...
          results[[ct]][[ds]] <- do.call(rbind, test_results)
        }
      }
    }
    
    # Combine results for cell type
    if(length(results[[ct]]) > 0) {
      results[[ct]] <- do.call(rbind, results[[ct]])
      results[[ct]]$p_adj <- p.adjust(results[[ct]]$p_value, method = "BH")
    }
  }
...
# 1. Basic boxplot for specific genes
genes_of_interest <- c('STK10')  # replace with your genes of interest
boxplot <- create_expression_boxplots(bulk, genes_of_interest)
print(boxplot)

# 3. Test for significant differences
test_results <- test_gene_expression(bulk, genes_of_interest)

# Print significant results
for(ct in names(test_results)) {
  if(length(test_results[[ct]]) > 0) {
    sig_results <- test_results[[ct]][test_results[[ct]]$p_adj < 0.05,]
    if(nrow(sig_results) > 0) {
      cat("\nSignificant differences in", ct, ":\n")
      print(sig_results)
    }
  }
}
