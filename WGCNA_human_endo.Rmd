...
Endothelial_cells_v1 - Endothelial_cells_v1 human
...
atlas <- SetupForWGCNA(
  ...
  # group_by = 'annotation',
  wgcna_name = "Endothelial_cells_v1" # the name of the hdWGCNA experiment
)
```
...
atlas <- ConstructNetwork(
  atlas,
  tom_name = 'Endo_HUMAN', # name of the topoligical overlap matrix written to disk
  overwrite_tom = TRUE
)
```
...
atlas <- ModuleConnectivity(
  atlas,
  group.by = 'Annotation', group_name = "Endothelial_cells"
)
```
...
ModuleRadarPlot(
  seu,
  group.by = 'less18M',
  barcodes = seu@meta.data %>% subset(Annotation == 'Endothelial_cells') %>% rownames(),
  axis.label.size=4,
  grid.label.size=4
)
```
...
```{r}
group1 <- atlas@meta.data %>% subset(Annotation == 'Endothelial_cells' & less18M == TRUE) %>% rownames
group2 <- atlas@meta.data %>% subset(Annotation == 'Endothelial_cells' & less18M == FALSE) %>% rownames
...
DMEs <- FindDMEs(
  ...
  barcodes2 = group2,
  test.use='wilcox',
  wgcna_name='Endothelial_cells_v1',
...
```{r eval=FALSE, include=FALSE}
qsave(atlas, paste0(PREPRDATADIR, 'atlas_WGCNA_endo.qs'))
```

```{r}
stromal <- subset(atlas, Annotation == 'Endothelial_cells')
stromal$brown_score <- stromal@misc$Endothelial_cells_v1$module_scores$brown
VlnPlot(stromal, features = 'brown_score')
VlnPlot(stromal, features = 'brown_score', group.by = 'less18M')
DimPlot(stromal, cols = my_colors)
DimPlot(stromal, group.by = 'less18M', cols = my_colors)
FeaturePlot(stromal, 'brown_score')
```

```{r}
stromal$green_score <- stromal@misc$Endothelial_cells_v1$module_scores$green
VlnPlot(stromal, features = 'green_score')
VlnPlot(stromal, features = 'green_score', group.by = 'less18M')
DimPlot(stromal, cols = my_colors)
DimPlot(stromal, group.by = 'less18M', cols = my_colors)
FeaturePlot(stromal, 'green_score')
```

```{r}
stromal$green_score <- stromal@misc$Endothelial_cells_v1$MEs$green
VlnPlot(stromal, features = 'green_score')
VlnPlot(stromal, features = 'green_score', group.by = 'less18M')
DimPlot(stromal, cols = my_colors)
DimPlot(stromal, group.by = 'less18M', cols = my_colors)
...
```{r}
stromal$yellow_score <- stromal@misc$Endothelial_cells_v1$module_scores$yellow
VlnPlot(stromal, features = 'yellow_score')
VlnPlot(stromal, features = 'yellow_score', group.by = 'less18M')
DimPlot(stromal, cols = my_colors)
DimPlot(stromal, group.by = 'less18M', cols = my_colors)
FeaturePlot(stromal, 'yellow_score')
```
...
for (gene in genes) {
  print(VlnPlot(stromal, features = gene, group.by = 'less18M'))
}
```
