...
expression_data <- FetchData(malig_cells, vars = gene)
  
# Get cluster information
clusters <- malig_cells$RNA_snn_res.0.2

# Create a data frame for plotting
plot_data <- data.frame(
  cell = names(expression_data),
  expression = as.numeric(expression_data$Mycn),
  cluster = factor(clusters)
)

# Calculate percentage of non-expressing cells
pct_zero <- mean(expression_data == 0) * 100
  
ggplot(plot_data) +
    # Add density plot in background
    geom_density(aes(x = expression), fill = "grey90", alpha = 0.3) +
    # Add points colored by cluster
    geom_point(aes(x = expression, y = 0, color = cluster), 
              position = position_jitter(height = 0.1),
              size = 2, 
              alpha = 0.2) +
    # Add rug plot
    geom_rug(aes(x = expression, color = cluster), 
             sides = "b", 
             alpha = 0.3) +
    # Customize theme
    theme_minimal() +
    ...
    labs(
      title = paste(gene, "Expression Distribution by Cluster"),
      subtitle = sprintf("%.1f%% cells show no expression", 
                        pct_zero),
...
```{r echo=FALSE}
malig_cells$zero_expression <- expression_data == 0

DimPlot(malig_cells, group.by = 'zero_expression', cols = c('grey', 'red'))
malig_cells_mycnfilt <- subset(malig_cells, zero_expression == FALSE)
```

## Mycn expression after filtration

```{r fig.height=4, fig.width=10}
dittoBoxPlot(subset(malig_cells, zero_expression == FALSE), "Mycn", group.by = "age", split.by = 'RNA_snn_res.0.2', color.panel = c("#6495EDFF", "#FF69B4FF"), 
              boxplot.width = 0.4, split.nrow = 1, boxplot.lineweight = 0.5)
```

```{r}
FeaturePlot(subset(malig_cells, zero_expression == FALSE), "ECM_Module1")
dittoBoxPlot(subset(malig_cells, zero_expression == FALSE), "ECM_Module1", group.by = "age", split.by = 'RNA_snn_res.0.2', color.panel = c("#6495EDFF", "#FF69B4FF"), 
             jitter.size = NA, boxplot.width = 0.4, split.nrow = 1, boxplot.lineweight = 0.5)
```

```{r eval=FALSE, include=FALSE}
malig_cells_mycnfilt <- malig_cells_mycnfilt %>% 
  NormalizeData() %>% 
...
## New clusterization with regression to CellCycle

```{r fig.height=5, fig.width=16,}
Idents(malig_cells_mycnfilt) <- malig_cells_mycnfilt$RNA_snn_res.0.2
plot_grid(DimPlot(malig_cells_mycnfilt, group.by = 'RNA_snn_res.0.2', cols = my_colors) +
  ggtitle("Clusters"), 
  DimPlot(malig_cells_mycnfilt, group.by = 'age', cols = c("#6495EDFF", "#FF69B4FF")) +
  ggtitle("Age"),
  DimPlot(malig_cells_mycnfilt, group.by = 'Phase', cols = my_colors) +
  ggtitle("Phase"),
  nrow = 1)
```
...
```{r fig.height=4, fig.width=10}
dittoBoxPlot(malig_cells_mycnfilt, "Mycn", group.by = "age", split.by = 'RNA_snn_res.0.2', color.panel = c("#6495EDFF", "#FF69B4FF"), 
              boxplot.width = 0.4, split.nrow = 1, boxplot.lineweight = 0.5)
```
...
```{r fig.height=7, fig.width=12}
Idents(malig_cells_mycnfilt) <- 'RNA_snn_res.0.2'

maligcells.markers <- FindAllMarkers(malig_cells_mycnfilt, only.pos = TRUE, min.pct = 0.25)
top5 <- maligcells.markers %>%
  dplyr::filter(p_val_adj < 0.05) %>% 
  group_by(cluster) %>%
  top_n(n = 7, wt = avg_log2FC)

DotPlot(malig_cells_mycnfilt, c('Mycn', top5$gene), cols = c('lightgrey', "#BA55D3FF")) +
  RotatedAxis()
```
...
```{r}
FeaturePlot(malig_cells_mycnfilt, "ECM_Module1")
dittoBoxPlot(malig_cells_mycnfilt, "ECM_Module1", group.by = "age", split.by = 'RNA_snn_res.0.2', color.panel = c("#6495EDFF", "#FF69B4FF"), 
             jitter.size = NA, boxplot.width = 0.4, split.nrow = 1, boxplot.lineweight = 0.5)
```

## New clusterization without regression to CellCycle

```{r include=FALSE}
malig_cells_mycnfilt2 <- malig_cells_mycnfilt %>% 
  NormalizeData() %>% 
  FindVariableFeatures() %>% 
  ScaleData() %>% 
  RunPCA(verbose=FALSE) %>% 
...
```{r fig.height=5, fig.width=16,}
Idents(malig_cells_mycnfilt2) <- malig_cells_mycnfilt2$RNA_snn_res.0.2
plot_grid(DimPlot(malig_cells_mycnfilt2, group.by = 'RNA_snn_res.0.2', cols = my_colors) +
  ggtitle("Clusters"), 
  DimPlot(malig_cells_mycnfilt2, group.by = 'age', cols = c("#6495EDFF", "#FF69B4FF")) +
  ggtitle("Age"),
  DimPlot(malig_cells_mycnfilt2, group.by = 'Phase', cols = my_colors) +
  ggtitle("Phase"),
  nrow = 1)
```
...
```{r fig.height=4, fig.width=10}
dittoBoxPlot(malig_cells_mycnfilt2, "Mycn", group.by = "age", split.by = 'RNA_snn_res.0.2', color.panel = c("#6495EDFF", "#FF69B4FF"), 
              boxplot.width = 0.4, split.nrow = 1, boxplot.lineweight = 0.5)
```
...
```{r fig.height=7, fig.width=12}
Idents(malig_cells_mycnfilt2) <- 'RNA_snn_res.0.2'

maligcells.markers <- FindAllMarkers(malig_cells_mycnfilt2, only.pos = TRUE, min.pct = 0.25)
top5 <- maligcells.markers %>%
  dplyr::filter(p_val_adj < 0.05) %>% 
  group_by(cluster) %>%
  top_n(n = 7, wt = avg_log2FC)

DotPlot(malig_cells_mycnfilt2, c('Mycn', top5$gene), cols = c('lightgrey', "#BA55D3FF")) +
  RotatedAxis()
```
...
```{r}
FeaturePlot(malig_cells_mycnfilt2, "ECM_Module1")
dittoBoxPlot(malig_cells_mycnfilt2, "ECM_Module1", group.by = "age", split.by = 'RNA_snn_res.0.2', color.panel = c("#6495EDFF", "#FF69B4FF"), 
             jitter.size = NA, boxplot.width = 0.4, split.nrow = 1, boxplot.lineweight = 0.5)
```