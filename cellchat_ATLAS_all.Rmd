...
```{r}
seu$Annotation_neutrophils <- seu$Annotation
seu$Annotation_neutrophils[seu$is.pure_Neutrophils == "Pure" | seu$celltype_humatlas_main == "Neutrophils"] <- "Neutrophils"
```
...
cellchat_young <- createCellChat(object = subset(seu, samples == 'TRUE'), group.by = "Annotation_neutrophils", assay = "RNA")
```
...
```{r eval=FALSE, include=FALSE}
seu$samples <- seu$less18M
cellchat_adult <- createCellChat(object = subset(seu, samples == 'FALSE'), group.by = "Annotation_neutrophils", assay = "RNA")
```
...
qsave(object.list, paste0(PREPRDATADIR, 'cellchat_list_neutrophils_patel.qs'))
```

```{r include=FALSE}
object.list <- qread(paste0(PREPRDATADIR, 'cellchat_list_neutrophils_patel.qs'))
```
...
```{r eval=FALSE, include=FALSE}
qsave(cellchat, paste0(PREPRDATADIR, 'cellchat_neutrophils_patel.qs'))
```

```{r include=FALSE}
cellchat <- qread(paste0(PREPRDATADIR, 'cellchat_neutrophils_patel.qs'))
```
