...
bulk <- AggregateExpression(atlas,
                            return.seurat = T, slot = "counts", assays = "RNA", 
                            group.by = c("Final_Annotation", "SampleID", "less18M"))
Idents(bulk) <- "less18M"
...
qsave(bulk, paste0(PREPRDATADIR, 'bulk_norep_ATLAS_deep.qs'))
```

```{r}
bulk <- qread(paste0(PREPRDATADIR, 'bulk_norep_ATLAS_deep.qs'))
```
...
# By cell type
DimPlot(bulk, reduction = "pca", group.by = "Final_Annotation", split.by = "dataset") + 
...
run_deg_analysis <- function(bulk) {
  ...
  valid_combinations <- table(bulk$Final_Annotation, bulk$dataset)
  ...
  for(celltype in unique(bulk$Final_Annotation)) {
...
perform_meta_analysis <- function(all_results, celltype) {
  ...
  if(length(common_genes) == 0) {
    print(paste("Zero common genes were found for", celltype))
    return(NULL)
  }
...
create_patient_heatmaps_genes <- function(bulk, genes) {
  ...
  for(celltype in unique(bulk$Final_Annotation)) {
    ...
    valid_combos <- table(bulk$Final_Annotation, bulk$dataset)
    valid_datasets <- names(valid_combos[celltype,valid_combos[celltype,] > 0])
    ...
    for(dataset_name in valid_datasets) {
      ...
      # Subset data
      subset_data <- subset(x = bulk, 
                           subset = Final_Annotation == celltype & dataset == dataset_name)
