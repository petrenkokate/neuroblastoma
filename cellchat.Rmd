...
```{r include=FALSE}
object.list <- qread(paste0(PREPRDATADIR, 'cellchat_list_mouse.qs'))
```
...
```{r include=FALSE}
cellchat <- qread(paste0(PREPRDATADIR, 'cellchat_mouse.qs'))
```
...
gg <- list()
for (i in 1:length(object.list)) {
  gg[[i]] <- netAnalysis_signalingRole_scatter(object.list[[i]], title = names(object.list)[i], weight.MinMax = weight.MinMax) + ylim(0.9,5) + xlim(0.2,6)
}
...
gg3 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Fibroblasts")

gg4 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Endothelial cells")
