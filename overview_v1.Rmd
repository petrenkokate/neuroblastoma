...
I perfomed differential expression analysis in immune cells by age. 

```{r fig.height=8, fig.width=15}
t_cells.de.markers <- read.csv(paste0(PREPRDATADIR, 't_cells_de_markers.csv'), row.names = 1)
b_cells.de.markers <- read.csv(paste0(PREPRDATADIR, 'b_cells_de_markers.csv'), row.names = 1)
myeloid.de.markers <- read.csv(paste0(PREPRDATADIR, 'myeloid_de_markers.csv'), row.names = 1)
...
plot_grid('',
          enrichplot_neg(b_cells.de.markers) +
            ggtitle('B-cells'),
...
```{r}
t_cells <- qread(paste0(PREPRDATADIR, 't_cells.qs'))
b_cells <- qread(paste0(PREPRDATADIR, 'b_cells.qs'))
myeloid <- qread(paste0(PREPRDATADIR, 'myeloid_cells.qs'))
```
...
heatmap_tcells_neg <- dittoHeatmap(t_cells, t_cells.de.markers %>% 
                            ...
                            dplyr::arrange(desc(avg_log2FC)) %>% 
                            rownames(),
             order.by = 'less18M', annot.by = c('less18M'), silent=T,
             annot.colors = c("#87CEEBFF", "#DDA0DDFF"), main = 'T cells', 
             breaks=seq(-2, 2, 0.2), heatmap.colors = colorRampPalette(c("#6495EDFF", "white", "#FF69B4FF"))(length(seq(-2, 2, 0.2)))) 
...
heatmap_bcells_neg <- dittoHeatmap(b_cells, b_cells.de.markers %>% 
                            ...
                            rownames(),
             order.by = 'less18M', annot.by = c('less18M'), silent=T,
             annot.colors = c("#87CEEBFF", "#DDA0DDFF"), main = 'B cells', 
             breaks=seq(-2, 2, 0.2), heatmap.colors = colorRampPalette(c("#6495EDFF", "white", "#FF69B4FF"))(length(seq(-2, 2, 0.2)))) 
...
heatmap_myeloid_neg <- dittoHeatmap(myeloid, myeloid.de.markers %>% 
                            ...
                            rownames(),
             order.by = 'less18M', annot.by = c('less18M'), silent=T,
             annot.colors = c("#87CEEBFF", "#DDA0DDFF"), main = 'Myeloid cells',
             breaks=seq(-2, 2, 0.2), heatmap.colors = colorRampPalette(c("#6495EDFF", "white", "#FF69B4FF"))(length(seq(-2, 2, 0.2)))) 
