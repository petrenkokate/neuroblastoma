...
# qsave(t_cells, paste0(PREPRDATADIR, 't_cells_mouse.qs'))
```
## Scatter plots
...
```{r}
nk_cells.de.markers %>% 
  filter(rownames(.) == 'Pdcd1')
  # arrange(avg_log2FC) %>% 
  # dplyr::filter(p_val_adj < 0.05)
```

```{r}
VlnPlot(t_cells, 'Havcr2')
```
...
ego_nkcells_pos <- enrichGO(gene = nk_cells.de.markers %>% 
...
barplot(ego_nkcells_pos, showCategory=20) +
  ggtitle('NK (positive)')

ego_nkcells_neg <- enrichGO(gene = nk_cells.de.markers %>% 
                            filter(p_val_adj < 0.05 & avg_log2FC < -0.5) %>% 
                            rownames() %>% 
                            mapIds(org.Mm.eg.db, 
                                   keys = .,
...
barplot(ego_nkcells_neg, showCategory=20) +
  ggtitle('NK (negative)')
```
...
```{r  fig.height=5, fig.width=12}
b_cells <- subset(seu, annotation == 'B_cells')
...
Idents(b_cells) <- b_cells$RNA_snn_res.0.2
plot_grid(DimPlot(b_cells, group.by = 'RNA_snn_res.0.2', cols = my_colors) +
  ggtitle("Clusters"), 
  DimPlot(b_cells, group.by = 'age', cols = c("#6495EDFF", "#FF69B4FF")) +
  ggtitle("Age"),
  nrow = 1)
```

```{r fig.height=4, fig.width=13}
plot_grid(dittoBarPlot(b_cells, 'age', group.by = 'RNA_snn_res.0.2', color.panel = my_colors),
          dittoBarPlot(b_cells, 'Phase', group.by = 'RNA_snn_res.0.2', color.panel = my_colors),
          dittoBarPlot(b_cells, 'RNA_snn_res.0.2', group.by = 'age', color.panel = my_colors),
          rel_widths = c(1, 1, 0.7),
          nrow = 1)
```
...
bcells.clusters.markers <- FindAllMarkers(b_cells, only.pos = TRUE, min.pct = 0.25)
top5 <- bcells.clusters.markers %>%
  dplyr::filter(p_val_adj < 0.05) %>% 
  group_by(cluster) %>%
  top_n(n = 7, wt = avg_log2FC)

DotPlot(b_cells, top5$gene, cols = c('lightgrey', "#BA55D3FF")) +
  RotatedAxis()
```
...
pred_main <- SingleR(test = sce, ref = mouse_ref_imm, 
                     labels = mouse_ref_imm$label.main, 
                     BPPARAM=MulticoreParam(10))
b_cells[['pred_main']] <- pred_main$pruned.labels
pred_fine <- SingleR(test = sce, ref = mouse_ref_imm, 
                     labels = mouse_ref_imm$label.fine, 
                     BPPARAM=MulticoreParam(10))
b_cells[['pred_fine']] <- pred_fine$pruned.labels

DimPlot(b_cells, group.by = 'pred_main')
```

```{r fig.height=10}
ego_bcells <- enrichGO(gene          = bcells.clusters.markers %>% 
                            filter(p_val_adj < 0.05) %>% 
...
b_cells$de_annotation <- Idents(b_cells)
...
b_cells <- RenameIdents(b_cells, new.cluster.ids)
b_cells$Annotation <- Idents(b_cells)

DimPlot(b_cells, cols = my_colors)
```

```{r fig.height=4, fig.width=13}
plot_grid(dittoBarPlot(b_cells, 'age', group.by = 'Annotation', color.panel = my_colors),
          dittoBarPlot(b_cells, 'Phase', group.by = 'Annotation', color.panel = my_colors),
          dittoBarPlot(b_cells, 'Annotation', group.by = 'age', color.panel = my_colors),
          rel_widths = c(1, 1, 0.7),
          nrow = 1)

# qsave(b_cells, paste0(PREPRDATADIR, 'b_cells_mouse.qs'))
```
...
```{r}
Idents(malig_cells) <- malig_cells$age
VlnPlot(malig_cells, c('Lars2'))
```

```{r}
VlnPlot(malig_cells, c('Fos', 'Junb'))
```

```{r}
FeaturePlot(malig_cells, c('Fos', 'Junb'))
```

```{r eval=FALSE, include=FALSE}
library("pathview")
gene.data <- subset(seu_bulk, annotation == 'malignant-cells')


rn <- rownames(gene.data[['RNA']]$counts)
gene.data <- gene.data[['RNA']]$counts %>% 
  as.data.frame(row.names = rn) %>% 
  rowwise() %>%
  mutate(malignant_cells_adult = mean(c_across(contains("malignant-cells_adult"))),
         malignant_cells_young = mean(c_across(contains("malignant-cells_young")))) %>%
  dplyr::select(malignant_cells_adult, malignant_cells_young)

gene.data$gene <- rn %>% mapIds(org.Mm.eg.db, 
                                   keys = .,
                                   column = "ENTREZID",
                                   keytype = "SYMBOL")
gene.data <- gene.data %>% 
...
mmu04010  <- pathview(gene.data  = gene.data,
                     pathway.id = "mmu04010 ",
                     species    = "mouse")
```
...
# ```{r}
# 
# viewPathway("c-FOS activation by phospho ERK1/2", 
#             readable = TRUE, 
#             organism = 'mouse',
#             foldChange = geneList)
# 
...
```{r fig.height=9}
reactome_malcells_pos <- enrichPathway(gene=malignant.de.markers %>% 
                            filter(p_val_adj < 0.05 & avg_log2FC > 0.5) %>% 
                            rownames() %>% 
                            mapIds(org.Mm.eg.db, 
                                   keys = .,
...
reactome_malcells_neg <- enrichPathway(gene=malignant.de.markers %>% 
                            filter(p_val_adj < 0.05 & avg_log2FC < -0.5) %>% 
                            rownames() %>% 
                            mapIds(org.Mm.eg.db, 
                                   keys = .,
...
barplot(reactome_malcells_pos, showCategory=20) +
  ggtitle('Malignant (positive)')

# barplot(reactome_malcells_neg, showCategory=20) +
#   ggtitle('Malignant (negative)')

DEenrichRPlot(malig_cells, ident.1 = 'young', ident.2 = 'adult',
              max.genes = 1000,
              enrich.database = 'HDSigDB_Mouse_2021')

DEenrichRPlot(malig_cells, ident.1 = 'young', ident.2 = 'adult',
              max.genes = 1000,
              enrich.database = 'WikiPathways_2019_Mouse')
...
DEenrichRPlot(malig_cells, ident.1 = 'young', ident.2 = 'adult',
              max.genes = 1000,
              enrich.database = 'KEGG_2019_Mouse')
...
```{r}
library(TSCAN)
entropy <- perCellEntropy(as.SingleCellExperiment(malig_cells))
ent.data <- data.frame(cluster=malig_cells$age, entropy=entropy)
...
```{r}
dittoBarPlot(malig_cells, 'Phase', group.by = 'age', color.panel = my_colors)
```
